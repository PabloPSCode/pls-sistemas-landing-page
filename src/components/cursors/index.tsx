...
const TargetCursor: React.FC<TargetCursorProps> = ({
  ...
  useEffect(() => {
    ...
    const isWithinZone = (element: Element | null) =>
      !zoneSelector || Boolean(element && element.closest(zoneSelector));
    let isInZone = !zoneSelector;
    ...
    const moveHandler = (e: MouseEvent) => {
      const inZoneNow = isWithinZone(e.target as Element | null);
      if (inZoneNow !== isInZone) {
        isInZone = inZoneNow;
        gsap.to(cursor, {
          autoAlpha: inZoneNow ? 1 : 0,
          duration: 0.15,
          ease: 'power2.out',
          overwrite: 'auto'
        });
    ...
    const scrollHandler = () => {
      ...
      if (!isWithinZone(elementUnderMouse)) {
        isInZone = false;
        gsap.to(cursor, {
          autoAlpha: 0,
          duration: 0.15,
    ...
    const enterHandler = (e: MouseEvent) => {
      ...
      if (!isWithinZone(directTarget)) return;
  ...
  }, [targetSelector, zoneSelector, spinDuration, moveCursor, constants, hideDefaultCursor, isMobile, hoverDuration, parallaxOn]);
